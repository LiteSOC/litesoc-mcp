...
if (!API_KEY) {
    process.stderr.write("[litesoc-mcp] ERROR: LITESOC_API_KEY environment variable is required.\n" +
...
function formatError(err) {
    ...
    if (err instanceof PlanRestrictedError) {
        return ("This action requires a Pro or Enterprise plan. " +
 ...
 */
function planFooter(metadata) {
    const parts = [];
    if (metadata.plan)
        parts.push(`Plan: ${metadata.plan}`);
    if (metadata.retentionDays != null)
        parts.push(`Retention: ${metadata.retentionDays} days`);
    if (metadata.cutoffDate)
        parts.push(`Data available from: ${metadata.cutoffDate}`);
    if (!parts.length)
        return "";
    return `\n\n---\n_${parts.join(" · ")}_`;
}
// ── Tool: list_alerts ─────────────────────────────────────────────────────────
server.tool("list_alerts", "Fetch open security alerts from LiteSOC. Returns severity, alert type, actor ID, source IP, and creation time. " +
    ...
    try {
        ...
        const summary = `Found **${data.total}** alert(s) · showing ${data.data.length}` +
...
// ── Tool: analyze_alert ───────────────────────────────────────────────────────
server.tool("analyze_alert", "Fetch full forensic details for a specific alert by ID. " +
    ...
    try {
        ...
        const forensicSection = ["### Forensic Intelligence"];
        // Geo
        if (alert.source_ip) {
        ...
        const networkFields = [];
        if ("is_vpn" in meta)
            networkFields.push(`VPN: ${meta.is_vpn ?? "redacted (Free plan)"}`);
        if ("is_tor" in meta)
            networkFields.push(`Tor: ${meta.is_tor ?? "redacted (Free plan)"}`);
        if ("is_proxy" in meta)
            networkFields.push(`Proxy: ${meta.is_proxy ?? "redacted (Free plan)"}`);
        if ("is_datacenter" in meta)
            networkFields.push(`Datacenter: ${meta.is_datacenter ?? "redacted (Free plan)"}`);
        if (networkFields.length)
            forensicSection.push(`- **Network:** ${networkFields.join(" · ")}`);
        // Geo coords
        if ("latitude" in meta && "longitude" in meta) {
            if (meta.latitude && meta.longitude) {
                forensicSection.push(`- **Coordinates:** ${meta.latitude}, ${meta.longitude}`);
                forensicSection.push(`- **Map:** https://www.google.com/maps?q=${meta.latitude},${meta.longitude}`);
            }
            else {
        ...
        if (meta.city)
            forensicSection.push(`- **City:** ${meta.city}`);
        if (meta.country)
            forensicSection.push(`- **Country:** ${meta.country}`);
        if (meta.isp)
            forensicSection.push(`- **ISP:** ${meta.isp}`);
        if (meta.threat_score != null)
            forensicSection.push(`- **Threat Score:** ${meta.threat_score}/100`);
        const lines = [
            `## Alert: ${alert.title}`,
            "",
            `| Field | Value |`,
...
server.tool("get_recent_events", "Fetch the latest security event logs from LiteSOC. " +
    ...
    event_name: z
        .string()
        .optional()
        .describe("Filter by event type, e.g. 'auth.login_failed'. Supports all 26 standard events."),
    actor_id: z
        .string()
        .optional()
    ...
    try {
        ...
        const rows = data.data.map((ev) => {
            ...
            const networkNote = ev.is_vpn === null && ev.is_tor === null
        ...
        const summary = `Showing **${data.data.length}** of ${data.total} events` +
...
server.tool("resolve_incident", "Mark a LiteSOC security alert as resolved. Records your resolution action and internal notes for the SOC 2 audit trail. " +
    "Requires Pro or Enterprise plan. " +
    ...
    resolution_type: z
        ...
        .optional()
        .default("other")
        .describe("How the incident was handled. " +
    ...
    resolved_by: z
        .string()
        .optional()
        .describe("Who is resolving this alert. Can be a user ID, email address, or identifier like 'ai-agent'. " +
        "Logged in the SOC 2 audit trail."),
}, async ({ alert_id, resolution_type, notes, resolved_by }) => {
    try {
        const { data: result, metadata } = await litesoc.resolveAlert(alert_id, resolution_type, {
            notes,
